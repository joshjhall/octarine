...
_Amended 2026-09-10: CRIT-2 (LOCATION, gazetteer path), CRIT-3 (NRP), and CRIT-4 (AGE) have shipped without ML, and the Phase 6 NER strategy question is resolved — see [`architecture/ner-strategy.md`](./architecture/ner-strategy.md). Counts in the header line are as of the original generation date._
...
Octarine is behind on three categorical axes. **NER** is the largest single gap: Presidio's spaCy / Stanza / Transformers / GLiNER pipeline supplies `PERSON`, `LOCATION`, `ORGANIZATION`, `NRP`, and `DATE_TIME` across ~20 languages out of the box. Octarine has since closed `NRP`, `AGE`, and `LOCATION` without ML (lexicon and gazetteer paths — CRIT-2/3/4 below), leaving free-text **`PERSON` and `ORGANIZATION`** uncovered: `is_name()` remains a regex heuristic that admits in its own comment "High false positive rate." Per the Phase 6 decision ([`architecture/ner-strategy.md`](./architecture/ner-strategy.md)) octarine does not close that remainder in-tree — model-backed detection is reached through the `Recognizer` extension point instead. **Country coverage** has discrete holes — no `UK_NHS`, no `DE_TAX_ID` / `DE_ID` / `DE_PASSPORT` (and the rest of the 11-recognizer German pack), no `SE_PERSONNUMMER`, no Italian `VAT_CODE` / `DRIVER_LICENSE` / `IDENTITY_CARD` / `PASSPORT`, no `ES_PASSPORT`, no Turkey at all (`TR_NATIONAL_ID` + `TR_LICENSE_PLATE`), no `US_ITIN` as a distinct type, no `US_MBI`. **Algorithmic correctness** has a small set of concrete bugs surfaced by the domain reports: `validate_ssn` accepts 9xx-area numbers as SSNs (those are ITINs), `validate_bank_account` inverts Luhn logic so any Luhn-valid 8-17 digit string is rejected as a bank account (~10% false-reject rate), `AustraliaMedicare` ships with `is_high_risk=false` / `is_hipaa_protected=false` despite being the AU equivalent of US MBI / NHS, Bitcoin address detection is shape-only with no base58check or Bech32 polymod, India Aadhaar lacks the UIDAI palindrome reject, Finland HETU is missing the post-2023 century markers B-F / U-Y, Singapore UEN has no checksum, US Driver License covers only 4 states with a permissive 6-13 alphanumeric fallback, US Passport rejects valid legacy 9-digit numbers, the DEA / state-medical-license entity collides on `IdentifierType::MedicalLicense`. Multilingual context keywords are missing for 14 of 15 keyword-bearing types; 80 of 95 identifiers carry no context keywords at all.

The blockers to claiming "drop-in Presidio replacement" are: free-text `PERSON` / `ORGANIZATION` (out-of-core by decision — see below), the missing German / Swedish / Turkish / UK NHS / Italian completeness packs, ITIN + MBI for US healthcare, libphonenumber-grade phone parsing, Public Suffix List validation for email / URL, multilingual context keywords for the top 20 identifiers, and the handful of correctness bugs above. **Total focused effort to reach drop-in parity: 6-8 weeks.** The NER policy question is now **resolved** ([`architecture/ner-strategy.md`](./architecture/ner-strategy.md)): octarine ships a `Recognizer` trait rather than a model, so no ML sub-crate is on the core roadmap and no engineer-months are budgeted for one. Callers needing `PERSON` / `ORG` against free text plug in a recognizer.
...
- **Indian UPI VPA (`IN_UPI`)** — `IdentifierType::IndiaUpi` / `PiiType::IndiaUpi` in `primitives/identifiers/financial/detection/upi.rs`, classified `is_high_risk` + `is_pci_protected` (DPDP Act 2023 + RBI payment PII). Detection gates on a curated NPCI PSP-handle allowlist (`oksbi` / `paytm` / `ybl` / ...) so `alice@oksbi` matches while `alice@paytm.com` (an email) does not — the allowlist is the email/VPA discriminator, not a shape regex. Wired end-to-end (validation, redaction with `ShowPsp`, Layer 3 builder + shortcuts, observe redactor). **Octarine ships this first** — Presidio's `InUpiRecognizer` is still an open PR ([microsoft/presidio#2036](https://github.com/microsoft/presidio/pull/2036)).
...
- **Recommendation**: ~~Feature-gated NER in the core crate~~ — **superseded by the Phase 6 decision** ([`architecture/ner-strategy.md`](./architecture/ner-strategy.md)). Octarine does not close this in-tree: `PERSON` is reached through the `Recognizer` extension point (#520), implemented by a sibling crate or caller code. A core `ner` Cargo feature and a `Source::Ner` confidence variant are both rejected. `name.rs` stays as a cheap, high-false-positive heuristic and is not a `PERSON` recognizer. Source: `gap_personal_contact.md` Gap 1, `gap_context_locales.md` C3.
- **Effort**: N/A in core — the remaining work is a recognizer implementation outside `octarine-core` (speculative, #592).

### CRIT-2. LOCATION entity — **CLOSED (gazetteer path)**
...
- **Octarine**: structured patterns (`is_street_address`, `is_postal_code` across 10 countries, `is_gps_coordinate`) **plus** free-text city/country detection via the bundled gazetteer. Neighborhood, landmark, and building names are still not detected.
- **Impact**: HIPAA Safe Harbor geographic-subdivision removal is servable for cities and countries — "Patient lives in Phoenix" and "I'm visiting Japan" both redact. Regions phrased descriptively ("the Pacific Northwest") do not.
- **Recommendation**: **Gazetteer path SHIPPED** — `primitives/identifiers/location/detection/named.rs` + `primitives/identifiers/common/patterns/location/gazetteer.rs` (~280 countries, ~1,500 major cities, `aho-corasick` `LeftmostLongest`, ambiguous-name confidence downgrade), surfaced as `IdentifierType::NamedLocation` with its `PiiType` counterpart. Recall is bounded by the bundled lists: no neighborhoods, landmarks, or regions ("the Pacific Northwest"). Closing that remainder is a `Recognizer` job, not a core NER feature flag — see the Phase 6 decision. Source: `gap_personal_contact.md` Gap 2.
- **Effort**: Done (gazetteer). Remaining recall gap is out-of-core.

### CRIT-3. NRP entity (nationality / religion / political) — **CLOSED**
...
- **Octarine**: **Shipped** — `primitives/identifiers/personal/detection/nrp.rs` matches embedded lexicons (~200 nationalities, ~50 religions, ~30 political affiliations) via `aho-corasick`, word-boundary validated so `"Catholic"` does not fire inside `"Catholicism"`. `IdentifierType::Nationality` / `Religion` / `PoliticalAffiliation` and their `PiiType` counterparts are wired through the scanner domains.
- **Impact**: GDPR Article 9 special-category coverage is claimable via the lexicon path. Recall is bounded by the bundled lists — unlisted nationalities, denominations, and parties are missed.
- **Effort**: Done (lexicon path, no ML). Source: `gap_personal_contact.md` Gap 3.

### CRIT-4. AGE entity — **CLOSED**

- **Presidio**: `TransformersRecognizer` emits `AGE` entity. Detects "42-year-old", "age 65", "in his thirties".
- **Octarine**: **Shipped** — `primitives/identifiers/personal/detection/age.rs` detects numeric forms (`"42-year-old"`, `"age 65"`, `"42 y.o."`) and lexical decade forms (`"in his thirties"` → 30), with `IdentifierType::Age` + `PiiType::Age` and HIPAA Safe Harbor >89 support. Input is length-capped for ReDoS protection.
- **Impact**: Healthcare and insurance redaction paths that require HIPAA Safe Harbor §164.514(b)(2)(i)(B) age aggregation are now servable.
- **Effort**: Done (pattern path, no ML). Source: `gap_personal_contact.md` Gap 4.
...
### CRIT-8. Multilingual context keywords — only `ApiKey` has non-English coverage — PARTIALLY RESOLVED (core set)
...
- **Status**: **Partially resolved** — the mechanism and the core set landed;
  the long tail of country-scoped identifiers has not. Steps 1–2 (the
  `(IdentifierType, KeywordLanguage)` registry and per-language files) in #666;
  steps 3–4 (the top-20 core set across 15 non-English tables, plus the language
...
### Phase 3: Multilingual context keywords (1-2 weeks) — CORE SET COMPLETE
...
- ~~LOW-3: Add English context keywords to the 17+ identifiers currently at `&[]`.~~ Done in #666 (this diff does not touch `en.rs`) — guarded by `test_low3_backfill_non_empty`.

Still open after #667:

- The country-scoped identifiers listed under CRIT-8 § *Still open*
  (`KoreaFrn`/`Brn`/`DriverLicense`/`Passport`, `Australia*`, `Singapore*`,
  `IndiaGstin`, `SpainNie`) have no native keywords in any language.
- LOW-4 (lemmatization): matching is whole-word for space-delimited scripts, not
  lemmatized, so inflected forms in morphologically rich languages (Finnish,
  Polish, German compounds) are still missed.
...
### Phase 6: NER strategy decision — **RESOLVED**

Decided in [#441](https://github.com/joshjhall/octarine/issues/441); design doc:
[`architecture/ner-strategy.md`](./architecture/ner-strategy.md).

- **Octarine ships a `Recognizer` trait, not a model.** Model-backed detection is reached through that extension point — implemented by a sibling crate or caller code — and `octarine-core` takes on no ML runtime. The trait is defined by #520 (`analyze/recognizer.rs`); #518 adds `RemoteRecognizer` for hosted services.
- The `NerProvider` name is dropped: one extension point serves NER, LLM, and remote-service recognizers alike, and it lives in `analyze/`, not `observe::pii`.
- A core `ner` Cargo feature is **rejected** — [`architecture/crate-layout.md`](./architecture/crate-layout.md) R2/R3 sends any ONNX/libtorch runtime to a sibling crate. This supersedes the feature-flag recommendations in CRIT-1 and CRIT-2 above. A future `octarine-ner-*` sibling crate remains open (#592, on hold).
- **CRIT-3 (NRP), CRIT-4 (AGE), and CRIT-2 (LOCATION, gazetteer path) have shipped** without ML — `personal/detection/nrp.rs`, `personal/detection/age.rs`, and `location/detection/named.rs` + `common/patterns/location/gazetteer.rs`.
- Remaining uncovered after this phase: **free-text `PERSON` (CRIT-1) and `ORGANIZATION`**, by design.
